...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    ...
    QBCore.Functions.TriggerCallback('A7-scoreboard:server:GetConfig', function(config)
        ...
        if IsControlJustPressed(0, Config.OpenKey1) then
            if not scoreboardOpen then
                if not scoreboardOpen1 then
                    QBCore.Functions.TriggerCallback('A7-scoreboard:server:GetPlayersArrays', function(playerList)
                        QBCore.Functions.TriggerCallback('A7-scoreboard:server:GetActivity', function(cops, ambulance)
                            QBCore.Functions.TriggerCallback('A7-scoreboard:server:GetPoints', function(craft, dealer)
        ...
        elseif IsControlJustPressed(0, Config.OpenKey) then
            if not scoreboardOpen1 then
                if not scoreboardOpen then
                    ...
                    QBCore.Functions.TriggerCallback('A7-scoreboard:server:GetPlayersArrays', function(playerList)
                        QBCore.Functions.TriggerCallback('A7-scoreboard:server:GetActivity', function(cops, ambulance, judge, mechanic, realestate, cardealer, taxi, lawyer, tow, admin)
                            QBCore.Functions.TriggerCallback('A7-scoreboard:server:GetPoints', function(craft, dealer)
                                PlayerOptin = playerList
                                Config.CurrentCops = cops
...
RegisterNetEvent('A7-scoreboard:client:SetActivityBusy')
AddEventHandler('A7-scoreboard:client:SetActivityBusy', function(activity, busy)
